...
def main():
    ...
    ds = env.from_source(source, watermark_strategy=None, source_name="input")
    ...
    def transform(s: str) -> str:
        try:
            ...
            ts = o.get("ts") or datetime.datetime.utcnow().isoformat()
            return json.dumps({"key": key, "value": val, "ts": ts})
        except Exception:
            return json.dumps({"key": "bad", "value": 0.0, "ts": datetime.datetime.utcnow().isoformat()})
